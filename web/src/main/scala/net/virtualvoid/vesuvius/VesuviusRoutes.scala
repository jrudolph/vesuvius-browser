...
class VesuviusRoutes(config: AppConfig)(implicit val system: ActorSystem) extends VesuviusApi with TwirlSupport with SprayJsonSupport {
  ...
  lazy val requestedWorkInputs: Seq[(WorkItemInput, Filter)] =
    Seq(
      ...
      TimesformerScroll5_27112024_17_32Input -> (s => !s.segmentId.startsWith("mesh") && hasReasonableSize(s)), //(s => (s.scrollId == "172" && !s.segmentId.startsWith("mesh")) || hasReasonableSize(s)),
