...
object SegmentReference {
  ...
  case class LegacyScrollReference(scroll: Int, base: ScrollServerBase) {
    def toNew: ScrollReference = ScrollReference.byId(scroll).get
  }
  val newScrollReferenceFormat: RootJsonFormat[ScrollReference] = jsonFormat3(ScrollReference.apply)
  val legacyScrollReferenceFormat: RootJsonFormat[LegacyScrollReference] = jsonFormat2(LegacyScrollReference.apply)
  implicit val scrollReferenceFormat: RootJsonFormat[ScrollReference] =
    new RootJsonFormat[ScrollReference] {
      override def read(json: JsValue): ScrollReference =
        if (json.asJsObject.fields.contains("scrollId"))
          newScrollReferenceFormat.read(json)
        else
          legacyScrollReferenceFormat.read(json).toNew

      override def write(obj: ScrollReference): JsValue = newScrollReferenceFormat.write(obj)
    }

  case class LegacySegmentReference(base: ScrollServerBase, scroll: Int, segmentId: String) {
    def toNew: SegmentReference = SegmentReference(ScrollReference.byId(scroll).get, segmentId)
  }

  val newSegmentFormat = jsonFormat2(SegmentReference.apply)
  val legacySegmentFormat = jsonFormat3(LegacySegmentReference.apply)
  implicit val segmentReferenceFormat: RootJsonFormat[SegmentReference] =
    new RootJsonFormat[SegmentReference]:
      override def read(json: JsValue): SegmentReference =
        if (json.asJsObject.fields.contains("scrollRef"))
          newSegmentFormat.read(json)
        else
          legacySegmentFormat.read(json).toNew

      override def write(obj: SegmentReference): JsValue = newSegmentFormat.write(obj)
